...
- Sets the mode used when resolving resources.
  - `keep` (default) - Keeps resources that cannot be resolved and injects them using the `APKTOOL_MISSING_{resId}` nomenclature when needed.
  - `remove` - Removes resources that cannot be resolved.
  - `dummy` - Adds dummy resources for all missing resources using `APKTOOL_DUMMY_{resId}` nomenclature.
